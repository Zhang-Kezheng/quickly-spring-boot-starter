...
public class RequestDeviceUtils {
     ...
     */
    public static RequestDevice getRequestDevice() {
        String via = RequestUtils.getParam("Via");
        if (StringUtils.isNotBlank(via)) {
            for (String mobileGateWayHeader : MOBILE_GATE_WAY_HEADERS) {
                if (via.contains(mobileGateWayHeader)) {
                    return RequestDevice.MOBILE;
                }
            }
        }

        String userAgent = RequestUtils.getParam("user-agent");
        if (StringUtils.isNotBlank(userAgent)) {
            for (String mobileUserAgent : MOBILE_USER_AGENTS) {
                if (userAgent.contains(mobileUserAgent)) {
                    return RequestDevice.MOBILE;
                }
            }

            for (String pcHeader : PC_HEADERS) {
                if (userAgent.contains(pcHeader)) {
                    return RequestDevice.PC;
                }
            }
        }

        return RequestDevice.UNKNOWN;
    }
}
